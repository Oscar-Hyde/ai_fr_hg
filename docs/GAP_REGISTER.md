...
| CHAT-02 | Concurrent message sequence allocation races | 3 | Conversation | CLOSED — IMPLEMENTED | Conversation row `FOR UPDATE` allocator; `turn_id` on messages; unique `(conversation, sequence)` index. | Uniqueness + duplicate-index tests. Hosted concurrency on the real bench. |
