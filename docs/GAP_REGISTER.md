...
| ING-06 | Processing lacks progress/cancel/recovery UX | 4 | Ingestion + Jobs + Frontend | IN PROGRESS | Progress/cancel + stale-heartbeat reaper (`Failed`/`StaleWorker`, one enqueue). Hosted Server `32453526161` green. Desk reconnect / live RQ death remain. | Cancel + stale-reaper tests on Server; browser → Phase 7. |
