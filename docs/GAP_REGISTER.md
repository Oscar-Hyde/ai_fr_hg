...
| ING-06 | Processing lacks progress/cancel/recovery UX | 4 | Ingestion + Jobs + Frontend | IN PROGRESS | Durable progress, heartbeat, cancellation, native realtime updates, and explicit requeue recovery are implemented on this branch; real-bench worker failure/cancel/reconnect evidence remains required before closure. | Worker failure/cancel/reconnect/browser tests. |
