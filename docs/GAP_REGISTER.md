...
| TRN-05 | Back-translation accounting/issues are incomplete | 4 | Translation | IN PROGRESS | Verification tokens add to `total_tokens`; progress writes usage; timeout-after-generation and cancel-after-partial tests added. Hosted Server confirmation pending. | Sampling + timeout/cancel usage tests on Server. |
