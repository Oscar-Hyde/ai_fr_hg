...
| ING-06 | Processing lacks progress/cancel/recovery UX | 4 | Ingestion + Jobs + Frontend | IN PROGRESS | Progress/cancel + stale-heartbeat reaper (`Failed`/`StaleWorker`, one enqueue). Live heartbeat is not reaped; in-flight enqueue is a no-op. Hosted Server `32454563561` green. OS-level RQ kill / Desk reconnect remain Phase 7. | Cancel + reaper tests on Server; live process kill → Phase 7. |
