...
| Assistant/agents | **READY / HARDENING REQUIRED** | Message sequencing was reopened on 2026-08-21 after a real bench run produced duplicate sequences, and re-fixed: allocation is an atomic counter increment under a row lock, and the unique `(conversation, sequence)` index is now schema-owned so fresh installs actually get it (CHAT-02). Allocation under a stale row snapshot surfaces as a retryable error rather than a duplicate (CHAT-09, open, Phase 7). Latest-N history, turn_id, route-state restore, focused document, fallback answer, footnote citations, rename/pin/archive/export, negative-feedback reason/correction, cooperative cancel/reconnect, and stable File identity on upload. Browser E2E remains Phase 7. |
