			...
			errors.append(
				{
					...
					"message": f"Payload {payload_bytes} bytes exceeds {MAX_PAYLOAD_BYTES}",
					"severity": "error",
				}
			)
			return False, errors
		...
		errors.append(
			{
				"field": "",
				"code": "too_deep",
				"message": f"Payload depth exceeds {MAX_DEPTH}",
				"severity": "error",
			}
		)
		return False, errors
		...
		errors.append(
			{
				...
				"message": f"Schema has {len(fields)} fields exceeds {MAX_FIELDS}",
			}
		)
		return False, errors
				...
				errors.append(
					{
						"field": name,
						"code": "type",
						"message": f"Field {name} expected array got {actual}",
						"expected": "array",
						"actual": actual,
						"severity": "error",
					}
				...
				errors.append(
					{
						"field": name,
						"code": "array_items",
						"message": f"Field {name} array items must be strings",
						"severity": "error",
					}
				)
			continue
				...
				errors.append(
					{
						"field": name,
						"code": "type",
						"message": f"Field {name} expected integer got {actual}",
						"expected": "integer",
						"actual": actual,
						"severity": "error",
					}
				...
				errors.append(
					{
						"field": name,
						"code": "type",
						"message": f"Field {name} expected number got {actual}",
						"expected": "number",
						"actual": actual,
						"severity": "error",
					}
				...
				errors.append(
					{
						"field": name,
						"code": "type",
						"message": f"Field {name} expected boolean got {actual}",
						"expected": "boolean",
						"actual": actual,
						"severity": "error",
					}
				...
				errors.append(
					{
						"field": name,
						"code": "type",
						"message": f"Field {name} expected string got {actual}",
						"expected": "string",
						"actual": actual,
						"severity": "error",
					}
					...
					errors.append(
						{
							"field": name,
							"code": "enum",
							"message": f"Field {name} value '{value}' not in allowed {allowed}",
							"expected": allowed,
							"actual": value,
							"severity": "error",
						}
					...
					errors.append(
						{
							"field": name,
							"code": "format",
							"message": f"Field {name} expected YYYY-MM-DD got '{value}'",
							"severity": "error",
						}
					)
...
def assert_valid(data, schema_doc):
	...
	if not ok:
		...
		raise ValidationError(
			_("Structured output failed schema validation: {0}").format(
				errors[0]["message"] if errors else "invalid"
			),
			errors=errors,
