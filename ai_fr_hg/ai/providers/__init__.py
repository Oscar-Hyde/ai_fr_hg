...
def get_provider(provider: str | None = None) -> BaseProvider:
	...
	if doc.provider_type == "Custom":
		if not doc.adapter_path:
			frappe.throw(_("AI Provider {0} has no Custom Adapter Path set.").format(doc.name))
		try:
		...
		if not provider_class:
			frappe.throw(_("No adapter is registered for provider type {0}.").format(doc.provider_type))

	if not issubclass(provider_class, BaseProvider):
		frappe.throw(_("Adapter for {0} must derive from BaseProvider.").format(doc.name))
...
def get_failover_providers(exclude: str | None = None) -> list[str]:
	...
	rows = frappe.get_all(
		"AI Provider", filters=filters, fields=["name"], order_by="priority asc, creation asc"
	)
	return [row.name for row in rows if row.name != exclude]
