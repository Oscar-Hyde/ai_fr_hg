...
from ai_fr_hg.ai.validation import ValidationError as _ValidationError
from ai_fr_hg.ai.validation import validate_extraction
...
def classify(
	...
	windows = (
		chunk_text(text, chunk_size=budget, chunk_overlap=200)
		if len(text) > budget
		else [type("W", (), {"content": text, "index": 0})()]
	)
	...
	if not votes:
		return {
			...
			"coverage": {
				...
				"coverage_ratio": round(processed_chars / source_chars, 4) if source_chars else 0,
				"strategy": "chunk_vote",
...
def extract_data(
	...
	if len(text) <= budget:
		prompt = EXTRACT_PROMPT.format(
			instructions=schema_doc.instructions or "", fields="\n".join(field_lines), text=text
		)
		result = run_chat(
			[{"role": "user", "content": prompt}],
			model=model_doc.name,
		...
		if data is None:
			raise _ValidationError(
				...
				provenance={
					"schema": schema_doc.name,
					"strict": bool(schema_doc.strict),
					"raw_preview": (result.content or "")[:500],
				},
			)
		...
		if not ok:
			raise _ValidationError(
				f"Structured output failed schema validation: {errors[0]['message'] if errors else 'invalid'}",
				errors=errors,
				provenance={
					"schema": schema_doc.name,
					"strict": bool(schema_doc.strict),
					"payload_bytes": len((result.content or "").encode("utf-8")),
		...
		if not ok2:
			raise _ValidationError(
				f"Coerced output failed validation: {errors2[0]['message']}",
				errors=errors2,
				provenance={"schema": schema_doc.name, "strict": bool(schema_doc.strict)},
			)
		# attach coverage for uniform API
		coerced["_coverage"] = {
			"source_chars": source_chars,
			"processed_chars": source_chars,
			"windows_total": 1,
			"windows_processed": 1,
			"windows_failed": 0,
			"coverage_ratio": 1.0,
	...
	for idx, w in enumerate(windows):
		...
		try:
			...
			if data is None or not isinstance(data, dict):
				raise _ValidationError(
					"Invalid JSON",
					errors=[{"field": "", "code": "malformed_json", "message": "Window invalid JSON"}],
					provenance={"window": idx},
				)
			ok, errors = validate_extraction(data, schema_doc)
			if not ok:
				raise _ValidationError("Window validation failed", errors=errors, provenance={"window": idx})
			coerced = _coerce_types(data, schema_doc)
			ok2, errors2 = validate_extraction(coerced, schema_doc)
			if not ok2:
				raise _ValidationError("Coerced window failed", errors=errors2, provenance={"window": idx})
			...
			provenance.append(
				{
					...
					"status": "failed",
					"error": str(ve)[:200],
					"errors": getattr(ve, "errors", []),
				}
			)
		except Exception as e:
			windows_failed += 1
			provenance.append(
				{"window_index": idx, "chars": len(content), "status": "failed", "error": str(e)[:200]}
			)
	if not per_window_results:
		raise _ValidationError(
			"All windows failed validation",
			errors=[
				{"field": "", "code": "all_windows_failed", "message": "No window produced valid extraction"}
			],
			provenance={"windows_total": windows_total, "windows_failed": windows_failed},
		)
	# Deterministic merge
	merged: dict = {}
	...
	if not okf:
		...
		raise _ValidationError(
			f"Merged output failed validation: {errorsf[0]['message']}",
			errors=errorsf,
			provenance={"merge_conflicts": merge_conflicts, "provenance": provenance},
		)
	merged["_coverage"] = {
		"source_chars": source_chars,
		"processed_chars": processed_chars,
...
def compare_documents(
	...
	if len(text_a) <= budget and len(text_b) <= budget:
		prompt = COMPARE_PROMPT.format(
			instructions=instructions, title_a=doc_a.title, text_a=text_a, title_b=doc_b.title, text_b=text_b
		)
		result = run_chat(
			[{"role": "user", "content": prompt}],
			model=model_doc.name,
		...
		return {
			...
			"model": model_doc.name,
			"coverage": {
				"source_chars_a": source_chars_a,
				"source_chars_b": source_chars_b,
				"processed_chars_a": source_chars_a,
				"processed_chars_b": source_chars_b,
				"windows_total_a": 1,
				"windows_total_b": 1,
				"windows_processed_a": 1,
				"windows_processed_b": 1,
				"windows_failed": 0,
				"coverage_ratio_a": 1.0,
				"coverage_ratio_b": 1.0,
				"coverage_ratio": 1.0,
				"strategy": "single_pass",
				"provenance": [],
			},
	...
	wins_a = (
		chunk_text(text_a, chunk_size=budget, chunk_overlap=200)
		if len(text_a) > budget
		else [type("W", (), {"content": text_a, "index": 0})()]
	)
	wins_b = (
		chunk_text(text_b, chunk_size=budget, chunk_overlap=200)
		if len(text_b) > budget
		else [type("W", (), {"content": text_b, "index": 0})()]
	)
	...
	for i in range(max_wins):
		...
		prompt = COMPARE_PROMPT.format(
			instructions=instructions, title_a=doc_a.title, text_a=ca, title_b=doc_b.title, text_b=cb
		)
		try:
			result = run_chat(
				[{"role": "user", "content": prompt}],
			...
			provenance.append(
				{"window_index": i, "chars_a": len(ca), "chars_b": len(cb), "status": "processed"}
			)
		except Exception as e:
			windows_failed += 1
			provenance.append(
	...
	if len(per_window_comparisons) == 1:
		...
		if len(combined) > budget * 2:
			...
			for batch in batches:
				...
				result = run_chat(
					[
						{
							"role": "user",
							"content": f"Combine these per-window comparisons into one coherent comparison.\n\n{combined_batch}",
						}
					],
					model=model_doc.name,
					operation="Compare",
			...
			combined = "\n\n".join(next_level)
			result = run_chat(
				[{"role": "user", "content": f"Synthesize final comparison from:\n{combined}"}],
				model=model_doc.name,
				operation="Compare",
				reference_doctype="AI Document",
				reference_name=document_a,
			...
			result = run_chat(
				[
					{
						"role": "user",
						"content": f"Synthesize these per-window comparisons into one final comparison:\n{combined}",
					}
				],
				model=model_doc.name,
				operation="Compare",
	...
	return {
		...
		"coverage": {
			"source_chars_a": source_chars_a,
			"source_chars_b": source_chars_b,
			"processed_chars_a": processed_a,
			"processed_chars_b": processed_b,
			"windows_total_a": len(wins_a),
			"windows_total_b": len(wins_b),
			"windows_processed_a": len(wins_a) - windows_failed if windows_failed < len(wins_a) else 0,
			"windows_processed_b": len(wins_b) - windows_failed if windows_failed < len(wins_b) else 0,
			"windows_failed": windows_failed,
			"coverage_ratio_a": coverage_ratio_a,
			"coverage_ratio_b": coverage_ratio_b,
			"coverage_ratio": round(min(coverage_ratio_a, coverage_ratio_b), 4),
			"strategy": "windowed_synthesis",
			"provenance": provenance,
			"note": "window alignment is for computational budgeting, not semantic section equivalence",
		},
	}
