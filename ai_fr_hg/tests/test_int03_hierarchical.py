...
def test_short_input_single_path():
	...
	with (
		patch("ai_fr_hg.ai.intelligence.resolve_model", return_value=m),
...
def test_provenance_markers_survive():
	...
	with (
		...
		intel.summarize(long_text, model="test-model")
		[c for c in rc.call_args_list if "REDUCE" in str(c) or "Section" in str(c)]
...
def test_provider_failure_propagates():
	...
	long_text = "hello " * 500
	with (
		patch("ai_fr_hg.ai.intelligence.resolve_model", return_value=m2),
		patch("ai_fr_hg.ai.intelligence.run_chat", side_effect=ProviderError("offline")),
	):
		try:
			intel.summarize(long_text, model="test-model")
