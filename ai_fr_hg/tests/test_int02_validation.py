...
from ai_fr_hg.ai.validation import MAX_DEPTH, MAX_PAYLOAD_BYTES, ValidationError, validate_extraction
...
def test_malformed_json_rejected_unit():
	...
	with (
		patch("ai_fr_hg.ai.intelligence.resolve_model") as mock_resolve,
...
def test_extra_field_non_strict():
	...
	ok, _err = validate_extraction({"invoice_number": "x", "amount": 1, "unexpected": "y"}, schema)
	assert ok
...
def test_nested_depth_bounded():
	...
	cur = deep
	for _i in range(MAX_DEPTH + 2):
		cur["nested"] = {"level": _i}
		cur = cur["nested"]
	_ok, _err = validate_extraction(deep, schema)
...
def test_provenance():
	...
	ok, _err = validate_extraction(data, schema)
	assert not ok
