...
| **Document intelligence** | 37 registered extensions through one pipeline: detect → extract → chunk → embed → index. Text-layer PDFs, Office Open XML, OpenDocument (including `.odp`), RFC `.eml`, text/code, CSV/JSON/XML/HTML, and images (vision or optional image OCR). Audio, video, database files, OLE `.doc`/`.xls`/`.ppt`, Outlook `.msg`, scanned-PDF OCR, and generic `zip`/`tar` sources are not supported. Extraction returns JSON; it does not create target DocType records. |
