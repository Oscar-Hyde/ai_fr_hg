...
| **Document intelligence** | 83 registered extensions through one pipeline: detect → extract → chunk → embed → index. Text-layer PDFs, Office Open XML (spreadsheet formulas preserved), OpenDocument (including `.odp`), RFC `.eml` with thread reconstruction, 40+ source-code languages, CSV/JSON/XML/HTML, images (vision or optional image OCR with confidence), and `zip`/`tar`/`gz`/`bz2`/`xz` archives (recursive, depth-limited, bounded). Audio, video, database files, OLE `.doc`/`.xls`/`.ppt` (ADR-012), Outlook `.msg`, and scanned-PDF OCR are not supported. Extraction returns JSON; it does not create target DocType records. |
| **Entity intelligence** | Deterministic pattern entities (email, url, phone, ip, hash, date, identifier, money) are exact. Optional semantic extraction (ADR-011) adds people, organizations, locations, concepts, and relationships through the governed model path; every result must be locatable verbatim in the source or it is discarded, and each carries a model confidence. Disabled by default because each scan costs a model call. |
